...
for(const id of SCIENCE_IDS)for(const style of ["midnight","paper","transparent"] as ScienceStyle[]){
 const html=renderToStaticMarkup(<ScienceObject id={id} label={SCIENCE_NAMES[id].fr} locale="fr" size={520} style={style} element={id==="science-comparator"?"Fe":id==="science-atom"?"Ru":"C"} compareElement="O" thumbnail/>);
...
const manifestUrl=new URL("../public/objects/science-v1/manifest.json",import.meta.url),manifest=JSON.parse(readFileSync(manifestUrl,"utf8")) as {assets:Record<string,string>};
