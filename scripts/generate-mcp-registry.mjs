 ...
 *
 * Le fichier est DÉRIVÉ (jamais écrit à la main) de deux artefacts déjà générés :
...
function main() {
  ...
  const components = catalogue.map((c) => {
    ...
    // Tokens de recherche : nom + description + catégorie + props + exemple.
    const haystack = [
      c.name,
      c.slug,
      c.description,
  ...
  const out = {
    ...
    source: "lib/generated/bpm-components.json + public/llms.txt",
  ...
  console.log(
    `Wrote ${path.relative(REPO_ROOT, OUT_FILE)} — ${components.length} composants ` +
      `(${withProps} avec props, ${withExample} avec exemple), ${categories.length} catégories.`,
  );
}
