...
for(const [name,file]of variants)for(const size of [224,288,384])for(const [id,activity]of [['sun',3],['earth',1],['jupiter',2],['saturn',2],['europa',0]]){
 ...
 const base='/objects/universe-v2',renderer=load(file).createCanvasPlanetRenderer(canvas,{surface:`${base}/compact/${id}.webp`,clouds:id==='earth'?`${base}/compact/earth-clouds.webp`:undefined,rings:id==='saturn'?`${base}/saturn-rings.png`:undefined,atmosphere:id==='sun'?[1,.5,.05]:id==='earth'?[.15,.5,1]:[0,0,0],star:id==='sun',activity});await renderer.ready;
