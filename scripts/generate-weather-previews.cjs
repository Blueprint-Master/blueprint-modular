...
const ids=['weather-sun','weather-fair','weather-overcast','weather-rain','weather-storm','weather-snow'],styles=['photorealistic','illustration'];
async function main(){
 ...
 if(process.argv.includes('--import-sun')){
  const offset=process.argv.indexOf('--import-sun');
  for(let i=0;i<2;i++)await sharp(process.argv[offset+1+i]).trim({background:{r:0,g:0,b:0,alpha:0}}).resize(430,430,{fit:'inside'}).extend({top:41,bottom:41,left:41,right:41,background:{r:0,g:0,b:0,alpha:0}}).resize(512,512,{fit:'contain',background:{r:0,g:0,b:0,alpha:0}}).webp({quality:90,alphaQuality:95,effort:6}).toFile(path.join(dir,`sun-${styles[i]}.webp`));
 }
 for(const style of styles)for(const kind of ['cloud','sun']){const file=`${kind}-${style}.webp`,data=fs.readFileSync(path.join(dir,file)),raw=await sharp(data).ensureAlpha().raw().toBuffer({resolveWithObject:true});textures[`${kind}-${style}`]={data:new Uint8ClampedArray(raw.data),width:raw.info.width,height:raw.info.height};assets.push({file,bytes:data.length,sha256:crypto.createHash('sha256').update(data).digest('hex'),width:512,height:512});}
 for(const id of ids)for(const style of styles){
  ...
  const texture=textures[`${id==='weather-sun'?'sun':'cloud'}-${style}`],renderer=mod.exports.createWeatherField(320,texture),pixels=Buffer.from(renderer.draw(id,style,id==='weather-storm'?1.35:id==='weather-sun'?1.5:0));
 ...
 if(process.argv.includes('--video'))for(const style of styles){
  const size=192,frames=288,fields=ids.map(id=>mod.exports.createWeatherField(size,textures[`${id==='weather-sun'?'sun':'cloud'}-${style}`])),width=size*ids.length;
 ...
 fs.writeFileSync(path.join(root,'docs/previews/weather/measurements.json'),JSON.stringify({date:'2026-09-11',environment:{node:process.version,platform:os.platform(),arch:os.arch(),cpu:os.cpus()[0].model},method:'Software RGBA renderer only; 36 warm samples per object/style. Excludes browser compositing. Not a mobile-device, network-transfer or battery measurement.',metrics},null,2)+'\n');
