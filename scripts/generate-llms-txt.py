...
Weather 1.0.0: `WeatherObject`, `WEATHER_IDS`, `WEATHER_OBJECTS` and `isWeatherId` are exported from the separate /objects entry. Five IDs: weather-fair, weather-overcast, weather-rain, weather-storm, weather-snow. `DISCOVERABLE_OBJECTS` now totals 33 objects. Both photorealistic and painted illustration materials are original AI-assisted assets, redistributed with attribution under Apache-2.0. Local cloud shape and density evolve; precipitation falls and fades; lightning forms at 1 s and dissipates by 2.8 s once per 24 s in selected views of CSS width ≥ 280 px, including constrained mobile; fixed storm posters show a lightning channel. Fixed thumbnails, lazy 512px WebP material, paused/offscreen/reduced-motion clocks stop. Software ceilings: 320px/18fps desktop, 224px/12fps constrained. Copy dist/assets/objects to public/objects. Exact weather references use version 1.0.0, not the Universe version 2.0.0; Maker must carry the matching weather parser and renderer. Weather-only sun remains deferred for quality. See docs/object-families.md and docs/weather-atmospheres.md. Visual quality is pending live-browser approval; these are draft assets, not an npm release.
Water 1.0.0: `WaterObject`, `WATER_IDS`, `WATER_OBJECTS` and `isWaterId` are exported from /objects. IDs: water-wave, water-ripple, water-waterfall, water-whirlpool. Each has photorealistic and painted materials generated for the family, with exact prompt, authorship, transformations and hashes in water-v1/generation.json and manifest.json. A procedural alpha/depth/foam field changes shape locally over a seamless 12 s cycle; this is not poster translation or a physical fluid simulation. Fixed thumbnails load one poster. A selected visible view loads one 384px material plus the lazy renderer, with 320px/18fps desktop and 224px/12fps constrained caps. Pause, hidden page, offscreen and reduced-motion stop the only clock. Exact references use 1.0.0 and Maker must package only the chosen poster/material plus credits. The catalogue totals 41 objects after Forms and Water. See docs/previews/water/README.md. Live-browser and physical-device approval remain required; these are draft candidates, not an npm release.
