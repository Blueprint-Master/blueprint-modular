...
Weather 1.0.0: `WeatherObject`, `WEATHER_IDS`, `WEATHER_OBJECTS` and `isWeatherId` are exported from the separate /objects entry. Six IDs: weather-sun, weather-fair, weather-overcast, weather-rain, weather-storm, weather-snow. `DISCOVERABLE_OBJECTS` now totals 38 objects. Photorealistic and painted illustration materials for the cloud and clear-weather sun are original AI-assisted assets, redistributed with attribution under Apache-2.0. Local cloud shape and density evolve; precipitation falls and fades; lightning forms at 1 s and dissipates by 2.8 s once per 24 s in selected views of CSS width ≥ 280 px, including constrained mobile; fixed storm posters show a lightning channel. The clear-weather sun keeps its disc anchored while irregular atmospheric sectors extend, lose density and resolve, with one fine caustic as secondary detail. Fixed thumbnails, lazy 512px WebP material, paused/offscreen/reduced-motion clocks stop. Software ceilings: 320px/18fps desktop, 224px/12fps constrained. Copy dist/assets/objects to public/objects. Exact weather references use version 1.0.0, not the Universe version 2.0.0; Maker must carry the matching weather parser and renderer. See docs/object-families.md, docs/weather-atmospheres.md and docs/previews/weather-sun/. Visual quality is pending live-browser approval; these are draft assets, not an npm release.
Water 1.0.0: `WaterObject`, `WATER_IDS`, `WATER_OBJECTS` and `isWaterId` are exported from /objects. IDs: water-wave, water-ripple, water-waterfall, water-whirlpool. Each has photorealistic and painted materials generated for the family, with exact prompt, authorship, transformations and hashes in water-v1/generation.json and manifest.json. A procedural alpha/depth/foam field changes shape locally over a seamless 12 s cycle; this is not poster translation or a physical fluid simulation. Fixed thumbnails load one poster. A selected visible view loads one 384px material plus the lazy renderer, with 320px/18fps desktop and 224px/12fps constrained caps. Pause, hidden page, offscreen and reduced-motion stop the only clock. Exact references use 1.0.0 and Maker must package only the chosen poster/material plus credits. The catalogue totals 42 objects after Weather, Forms and Water. See docs/previews/water/README.md. Live-browser and physical-device approval remain required; these are draft candidates, not an npm release.
Flora 1.0.0: `FloraObject`, `FLORA_IDS`, `FLORA_OBJECTS` and `isFloraId` are exported from /objects. IDs: flora-fern, flora-blossom, flora-meadow, flora-branch. Original procedural Canvas studies use synthetic studio and illustration treatments without third-party or raster source assets. The fern unfurls, the blossom opens, wind crosses the meadow and ginkgo leaves twist locally over an exact 12 s loop. Fixed SVG thumbnails; one lazy renderer and one active clock only for the selected visible object. Pause, hidden page, offscreen and reduced-motion stop it. Caps: 320px/18fps desktop, 224px/12fps constrained. Exact references use 1.0.0 and Maker must carry the matching parser and renderer.
Materials 1.0.0: `MaterialObject`, `MATERIAL_IDS`, `MATERIAL_OBJECTS` and `isMaterialId` are exported from /objects. IDs: material-crystal, material-geode, material-liquid-metal, material-dichroic-glass, material-obsidian. The catalogue totals 51 objects. Original procedural Canvas studies use synthetic studio and illustration treatments with no raster or third-party asset. Their local characteristic changes are crystal growth, geode crystallization, liquid-metal merge/split, dichroic-glass folding and obsidian crack branching/healing. One restrained moving light detail; exact 12 s loop. Fixed SVG thumbnails, lazy renderer, one selected visible clock. Pause, hidden page, offscreen and reduced-motion stop it. Caps: 320px/18fps desktop, 224px/12fps constrained. Exact references use 1.0.0; Maker must package the renderer and chosen poster. See docs/previews/materials/README.md. Browser/touch approval and npm publication remain separate gates.
