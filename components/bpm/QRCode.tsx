...
export function QRCode({ value, size = 128, fgColor = "var(--bpm-text-primary)", bgColor = "var(--bpm-bg-primary)", className = "" }: QRCodeProps) {
  if (!value) {
    return (
      <div className={"bpm-qrcode " + className} style={{ width: size, height: size, background: "var(--bpm-bg-secondary)", borderRadius: "var(--bpm-radius-sm)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--bpm-text-secondary)", fontSize: "var(--bpm-font-size-sm)" }}>
        Pas de valeur
      </div>
    );
  }
