...
export interface DataExplorerClassicProps {
...
export function DataExplorerClassic({
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter((row) => Object.values(row).some((v) => String(v ?? "").toLowerCase().includes(s)));
    }
    if (sortKey) {
      list = [...list].sort((a, b) => {
        const va = a[sortKey];
  ...
  return (
    ...
    >
      ...
      <div className="bpm-data-explorer-table-wrap" style={{ overflowX: "auto", paddingLeft: 12, paddingRight: 12 }}>
        <table
          style={{
            width: "100%",
        ...
        >
          ...
          <tbody>
            {paginated.map((row, i) => (
              <tr key={i} style={{ borderBottom: "1px solid var(--bpm-border)" }}>
                {columns.map((col, colIndex) => (
                  <td key={col.key} style={{ padding: "10px 12px", paddingLeft: colIndex === 0 ? 0 : 12 }}>
