 ...
 return <>
  ...
  <section className={styles.studio} aria-label={S.controls}>
   <div className={styles.stage}><span className={styles.stageLabel}>{S[item.family]} / {item.name[locale]}</span>
    ...
    <div className={styles.stageFooter}><span>{planet?S.hint:S.vector}</span>{planet&&<button type="button" aria-label={S.motion} aria-pressed={motion} onClick={()=>setMotion(!motion)}>{motion?<Pause size={18}/>:<Play size={18}/>}</button>}</div>
   </div>
   <div className={styles.details}><span className={styles.eyebrow}>{S.reuse}</span><h2>{item.name[locale]}</h2>
    {planet&&<div className={styles.tabs} role="group" aria-label="Style">{(["photorealistic","illustration"] as const).map(v=><button type="button" key={v} aria-pressed={variant===v} onClick={()=>{setVariant(v);setCopyState("idle");}}>{S[v]}</button>)}</div>}
    ...
    {planet&&<label className={styles.range}>{S.speed}<output>×{speed.toFixed(1)}</output><input aria-label={S.speed} type="range" min="0.1" max="3" step="0.1" value={speed} onChange={e=>{setSpeed(Number(e.target.value));setCopyState("idle");}}/></label>}
