...
import { ModularObject, EarthControls, DEFAULT_EARTH_LAYERS, type EarthLayers, DISCOVERABLE_OBJECTS, resolveModularObject, searchModularObjects, isPlanetId, isWeatherId, isFormId, isWaterId, isFloraId, isMaterialId, UNIVERSE_VERSION, planetProvenance, type PlanetStyle, type ObjectFamily } from "../../packages/core/src/objects";
...
export function ObjectsCatalogue(){
 ...
 const planet=isPlanetId(selected),weather=isWeatherId(selected),form=isFormId(selected),water=isWaterId(selected),flora=isFloraId(selected),material=isMaterialId(selected),animated=planet||weather||form||water||flora||material,item=resolveModularObject(selected,planet?UNIVERSE_VERSION:"1.0.0")!,version=item.version,provenance=planetProvenance(selected);
 ...
 return <>
  ...
  <section className={styles.studio} aria-label={S.controls}>
   <div className={styles.stage}><span className={styles.stageLabel}>{S[item.family]} / {item.name[locale]}</span>
    ...
    <div className={styles.stageFooter}><span>{material?S.materialsHint:flora?S.floraHint:water?S.waterHint:form?S.formsHint:weather?S.weatherHint:planet?S.hint:S.vector}</span>{animated&&<button type="button" aria-label={S.motion} aria-pressed={motion} onClick={()=>setMotion(!motion)}>{motion?<Pause size={18}/>:<Play size={18}/>}</button>}</div>
   </div>
   <div className={styles.details}><span className={styles.eyebrow}>{S.reuse}</span><h2>{item.name[locale]}</h2>
    ...
    {!weather&&!form&&!water&&!flora&&!material&&<label className={styles.range}>{S.angle}<output>{angle}°</output><input aria-label={S.angle} type="range" min="-35" max="35" value={angle} onChange={e=>{setAngle(Number(e.target.value));setCopyState("idle");}}/></label>}
    ...
    <p className={styles.note} role="status">{copyState==="download"?S.downloadHint:copyState==="error"?S.copyError:copyState==="done"?S.copied:material?S.materialsHint:flora?S.floraHint:water?S.waterHint:form?S.formsHint:weather?S.weatherHint:planet?S.art:S.vector}</p>
  ...
  {[{family:"space" as const,parent:undefined},...(["jupiter","saturn","uranus","neptune"] as const).map(parent=>({family:"space" as const,parent})),...families.filter(f=>f!=="space").map(family=>({family,parent:undefined}))].map(({family:f,parent})=>{const group=items.filter(o=>o.family===f&&o.parent===parent),heading=parent?`${S.space} · ${locale==="en"?"Moons of ":parent==="uranus"?"Lunes d’":"Lunes de "}${resolveModularObject(parent)!.name[locale]}`:S[f];return group.length>0&&<section key={parent??f} className={styles.collection} aria-label={heading}><div className={styles.collectionHeading}><h2>{heading}</h2><span>{group.length} {S.count}</span></div>
   <div className={styles.grid}>{group.map(o=><button type="button" className={styles.card} key={o.id} aria-pressed={o.id===selected} aria-label={o.name[locale]} onClick={()=>choose(o.id)}>
    <div className={`${styles.thumbnail} ${f==="space"||f==="weather"||f==="forms"||f==="water"||f==="flora"||f==="materials"?styles.cosmic:""}`}><ModularObject id={o.id} version={f==="space"?UNIVERSE_VERSION:o.version} variant={variant} locale={locale} size={220} thumbnail/></div>
