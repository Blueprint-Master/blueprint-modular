/**
 * Seed : crée 3 procédures de bonne tenue dans le Wiki (source TypeScript).
 * Version exécutable : prisma/seed-wiki-procedures.cjs
 * Usage : npm run seed:wiki  ou  npx prisma db seed
 * Prérequis : au moins un utilisateur en base (créer un compte puis lancer le seed).
 */
...
async function main() {
  ...
  for (const proc of PROCEDURES) {
    ...
    const data = {
      ...
      isPublished: true,
      excerpt: (proc as { excerpt?: string }).excerpt ?? null,
      tags: (proc as { tags?: string[] }).tags ?? [],
      wordCount,
