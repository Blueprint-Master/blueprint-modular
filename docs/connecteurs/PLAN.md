...
> **Statut : VALIDÉ le 2026-06-13.** Ce document a été le livrable du Chantier 0 ;
> les 5 arbitrages ci-dessous sont tranchés et l'implémentation est lancée (PR1).

---

## 0. Décisions validées (2026-06-13)

| # | Décision | Tranché |
|---|---|---|
| 1 | **Garde CI** dans `scripts/gate.cjs` (porte unique, pas de workflow dédié). La garde `check-connector-secrets.mjs` **atterrit dès la PR1** — elle doit précéder ce qu'elle garde. | Option (A) |
| 2 | **Pas d'entrée SiteNav** au MVP (cohérent avec l'omission de Composants/Modules). Les 3 piliers seront ajoutés ensemble à la consolidation nav de la vitrine. | Confirmé |
| 3 | **Surface en `app/(public)/(site)/connecteurs/`** (chrome vitrine, miroir de `/components`). Sépare physiquement le pilier public du module homonyme in-app. | Confirmé |
| 4 | **Lien Ω par convention d'`id`, sans toucher `app-spec.ts`.** Confirmé empiriquement : le type `Connector` de l'AppSpec est **IoT-only** (`protocol: mqtt\|opcua\|modbus\|rest\|websocket`) et n'est référencé **que** dans `packages/core/src/schema/` — aucun builder/Maker ne le consomme. | Confirmé |
| 5 | **Nom du pilier = « Connecteurs »** (nom stratégique du brief, route `/connecteurs`). Le **module d'ingestion homonyme sera renommé** plus tard (zone gelée → file différée, cf. §15). Au MVP, sans entrée nav (décision 2), les deux noms ne co-occurrent dans aucune navigation → on ne **livre** jamais deux « Connecteurs ». | Option (a) |

**Réserves attachées à la décision 4 :**
- Le type s'appelle **`ConnectorDescriptor`** (≠ `Connector` de l'AppSpec) — on ne réutilise pas
  `Ω.connectors` pour ce concept.
- La collision **IoT-connectors vs API-connectors** est consignée comme **item de curation Ω
  différé** (réconciliation human-gated, au moment du binding Maker) — voir §15.
- **Périmètre explicite** : ce chantier livre **catalogue + surface vitrine uniquement**. Le
  **binding Maker** (le LLM choisit → le builder câble dans `s`) est un **track ultérieur** ;
  « par convention d'`id` » ne masque pas que **`s` n'est pas encore touché**.
...
## 9. Fichiers partagés / zones gelées — conflits signalés (tranchés au §0)
...
3. **`scripts/gate.cjs`** (infra CI) — **modifié** (option A, §0) : ajout d'une étape garde
   secrets + exécution des tests connecteurs. Fichier d'infra CI, hors zones gelées
   modules/i18n/vitrine/cœur.
...
## 14. Questions ouvertes — TRANCHÉES

Les 4 questions du Chantier 0 + la décision de nom (5) sont résolues au **§0**. Plus aucune
question ouverte bloquante. L'implémentation suit le découpage du §11, à partir de la PR1.

---

## 15. File différée (items hors-MVP, tracés)

À reprendre après le MVP du pilier, dans l'ordre de dépendance :

1. **Renommage du module d'ingestion homonyme** (`app/(app)/modules/connecteurs/`) — zone gelée
   (chantier modules en cours). À planifier avec le propriétaire des modules, **avant** la
   consolidation nav de la vitrine (sinon deux « Connecteurs » deviendraient visibles ensemble).
2. **Consolidation nav vitrine** — ajout des 3 piliers (Composants / Modules / Connecteurs)
   ensemble dans `SiteNav.tsx` + dictionnaire i18n partagé (zone gelée).
3. **Binding Maker** (track ultérieur) — le LLM choisit un `id` de connecteur ; le builder le
   câble dans `s` (`Ω.connectors`). Implique : validateur Maker rejetant tout `id` hors
   catalogue, projection `responseMapping → Ω.connectors[].fields`, résolution `vault.get`.
4. **Réconciliation Ω IoT-connectors vs API-connectors** — item de **curation human-gated** :
   décider si `Connector` (protocole/IoT) et `ConnectorDescriptor` (API) convergent (ex. ajout
   d'un `descriptorId`/`auth` typé au schéma cœur) ou restent deux concepts. **Ne pas** trancher
   mécaniquement ; passe par le curateur, au moment du binding Maker.
