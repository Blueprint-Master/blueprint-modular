...
it("discovers materials and carries exact settings into the integration",()=>{const {container}=render(<ObjectsCatalogue/>);fireEvent.click(screen.getByRole("button",{name:"Cristaux & matières",exact:true}));fireEvent.click(screen.getByRole("button",{name:"Métal liquide",exact:true}));fireEvent.click(screen.getByRole("button",{name:"Dessin",exact:true}));fireEvent.change(screen.getByRole("slider",{name:"Vitesse de l’animation"}),{target:{value:"0.7"}});const code=container.querySelector("code")?.textContent;expect(code).toContain('id="material-liquid-metal" version="1.0.0"');expect(code).toContain('variant="illustration" playing={true} speed={0.7}');expect(screen.queryByRole("slider",{name:"Inclinaison"})).toBeNull();});
