...
it("withdraws rejected vegetation and material thumbnails from discovery",()=>{render(<ObjectsCatalogue/>);expect(screen.queryByRole("button",{name:"Végétation",exact:true})).toBeNull();expect(screen.queryByRole("button",{name:"Cristaux & matières",exact:true})).toBeNull();expect(screen.queryByRole("button",{name:"Fougère qui s’éveille",exact:true})).toBeNull();expect(screen.queryByRole("button",{name:"Métal liquide",exact:true})).toBeNull();});
it("discovers the layered scientific family and carries settings into integration",()=>{const {container}=render(<ObjectsCatalogue/>);fireEvent.click(screen.getByRole("button",{name:"Sciences & éléments",exact:true}));fireEvent.click(screen.getByRole("button",{name:"Tableau périodique",exact:true}));fireEvent.change(screen.getByRole("combobox",{name:"Élément"}),{target:{value:"Fe"}});fireEvent.click(screen.getByRole("checkbox",{name:"Repères"}));fireEvent.click(screen.getByRole("button",{name:"Bloc électronique"}));const code=container.querySelector("code")?.textContent;expect(code).toContain('id="science-periodic-table" version="1.0.0"');expect(code).toContain('variant="midnight"');expect(code).toContain('element="Fe"');expect(code).toContain('colorMode="block"');expect(code).toContain('"guides":false');});
