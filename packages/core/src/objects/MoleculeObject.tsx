...
import { useMoleculeMotion } from "./useMoleculeMotion";

export function MoleculeObject({ settings = DEFAULT_MOLECULE_SETTINGS, visualStyle, layers, locale, playing=true, thumbnail=false }: { settings?: MoleculeSettings; visualStyle: ScienceStyle; layers: ScienceLayers; locale: "fr" | "en"; playing?:boolean; thumbnail?:boolean }) {
  const valid = parseMoleculeSettings(settings), fr = locale === "fr";
  const ref=useRef<SVGSVGElement>(null);
  const motion=useMoleculeMotion(ref,!!valid?.motion && playing && !thumbnail && layers.structure,valid?.motionSpeed??1);
  ...
  return <svg ref={ref} role="img" aria-label={name} data-motion-active={motion.active} data-molecule-status={preset?.method === "computed" ? "computed-conformer" : preset ? "sourced-geometry" : "unvalidated"} viewBox="0 0 520 520" style={{ width: "100%", height: "100%", display: "block", color: visualStyle === "transparent" ? undefined : ink, fontFamily: "ui-sans-serif,system-ui,sans-serif" }}>
    ...
    {layers.identity && <g fill={ink}><text x="32" y="38" fontSize="9" letterSpacing="2" fill={muted}>{fr ? "MODÈLE MOLÉCULAIRE · BOULES ET BÂTONNETS" : "MOLECULAR MODEL · BALL AND STICK"}</text><text x="32" y="90" fontSize={graph.atoms.length>20?32:42} fontWeight="700">{preset?.formula ?? displayFormula(moleculeFormula(graph))}</text><text x="488" y="86" textAnchor="end" fontSize={name.length>22?12:15}>{name}</text></g>}
