...
export function AtomicObject({element,locale,visualStyle,layers,atomic}:{element:ElementDatum;locale:"fr"|"en";visualStyle:ScienceStyle;layers:ScienceLayers;atomic?:AtomicLayerSettings}){
  ...
  return <svg role="img" aria-label={(fr?"Analyseur atomique : ":"Atomic analyser: ")+element.name[locale]} viewBox="0 0 520 520" style={{display:"block",width:"100%",height:"100%",color:transparent?undefined:ink,fontFamily:"ui-sans-serif,system-ui,sans-serif"}}>
    ...
    {layers.analysis&&<g><path d="M32 122H488" stroke={muted} strokeOpacity=".25"/><text x="32" y="141" fontSize="9" letterSpacing="1" fill={muted}>{prediction?(fr?"CONFIGURATION PRÉDITE":"PREDICTED CONFIGURATION"):(fr?"CONFIGURATION DE RÉFÉRENCE":"REFERENCE CONFIGURATION")}</text><text x="32" y="165" fill={ink} fontSize={configuration.length>30?13:17} fontFamily="ui-monospace,monospace">{configuration||"—"}</text></g>}
    {layers.classification&&<g fill={muted}><text x="32" y="215" fontSize="9" letterSpacing="1">{fr?"COUCHES":"SHELLS"}</text>{populations.map((p,i)=><g key={p.n} transform={`translate(32 ${238+i*22})`}><text fontSize="10">n={p.n}</text><rect x="32" y="-8" width={Math.max(4,p.electrons*1.8)} height="7" rx="3" fill={positive} fillOpacity={subshell?.n===p.n?1:.35}/><text x="102" fontSize="11" fill={ink}>{p.electrons} e⁻</text></g>)}</g>}
    {layers.structure&&<g transform="translate(324 281) scale(.68)">{orbitals&&subshell?<AtomicOrbitalLayer n={subshell.n} l={subshell.l} density={s.density} nucleus={s.nucleus} opacity={s.opacity} positive={positive} negative={negative} guides={layers.guides}/>:s.representation==="sphere"?<AtomSphereLayer element={element.symbol} radius={96} opacity={s.opacity} labels={layers.identity}/>:<AtomicDensityLayer settings={s} accent={positive} ink={ink}/>}</g>}
    {layers.guides&&orbitals&&subshell&&<g fontSize="10" fill={muted}><text x="32" y="190">{subshell.id} · m = 0 · {fr?"coupe xz":"xz section"}</text><text x="190" y="398" fill={positive}>ψ +</text><text x="235" y="398" fill={negative}>ψ −</text><text x="286" y="398">{fr?"signes, pas charges":"signs, not charges"}</text></g>}
