...
export function createCanvasPlanetRenderer(canvas:HTMLCanvasElement,config:{surface:string;clouds?:string;rings?:string;atmosphere:readonly number[];star:boolean}){
  ...
  for(let y=0;y<size;y++)for(let x=0;x<size;x++){
   ...
   if(rr<=1){
    ...
    if(frame.illustrated){const lum=tex[0]*.299+tex[1]*.587+tex[2]*.114;const paper=(Math.sin(Math.floor(u*1600)*127.1+Math.floor(v*1600)*311.7)*43758.5453)%1;
     ...
     lighting=.35+.65*smooth(-.12,.8,px*light[0]+py*light[1]+z*light[2]);lighting=lighting*.78+Math.floor(lighting*7)/7*.22;}
    color=tex.slice(0,3).map(c=>c*(config.star?1.15:lighting));
    if(clouds){const cloud=smooth(.12,.85,sample(clouds,u+frame.rotation*1.045+.015,v)[0])*.86;color=color.map((c,i)=>c*(1-cloud)+[.92,.96,1][i]*lighting*cloud);}
    const rim=Math.pow(1-z,3.2)*(.35+.65*diffuse)*.65;color=color.map((c,i)=>c+config.atmosphere[i]*rim);alpha=1;
