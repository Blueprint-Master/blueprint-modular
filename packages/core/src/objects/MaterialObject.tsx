...
import {MATERIAL_ASSET_PATH,materialBudget,materialPosterPath,type MaterialId,type MaterialStyle} from "./materials";
export interface MaterialObjectProps{id:MaterialId;label:string;size?:number;style?:MaterialStyle;playing?:boolean;speed?:number;thumbnail?:boolean;assetBaseUrl?:string;}
/** Living material study. A fixed lazy SSR poster becomes one bounded canvas only in view. */
export function MaterialObject({id,label,size=360,style="photorealistic",playing=true,speed=1,thumbnail=false,assetBaseUrl=MATERIAL_ASSET_PATH}:MaterialObjectProps){
 ...
 useEffect(()=>{if(thumbnail||!visible||!pageVisible)return;const target=canvas.current;if(!target)return;let disposed=false;const media=window.matchMedia("(prefers-reduced-motion: reduce)");
  const constrained=window.matchMedia("(pointer: coarse)").matches||window.innerWidth<600||(navigator as Navigator&{connection?:{saveData?:boolean}}).connection?.saveData===true,budget=materialBudget(Math.min(width,safeSize),window.devicePixelRatio||1,constrained);
  let draw=()=>{};const clock=createPlanetClock(budget.fps,delta=>{const value=settings.current.speed;elapsed.current+=delta*(Number.isFinite(value)?Math.max(.1,Math.min(3,value)):1);draw();});
  ...
  void import("./material-renderer").then(({drawMaterial})=>{if(disposed)return;target.width=target.height=budget.size;const ctx=target.getContext("2d");if(!ctx)throw Error("Canvas unavailable");draw=()=>drawMaterial(ctx,id,style,elapsed.current,budget.size);refresh.current=update;media.addEventListener("change",update);setStatus("ready");update();
  }).catch(()=>{if(!disposed){clock.stop();setStatus("error");}});return()=>{disposed=true;clock.stop();media.removeEventListener("change",update);refresh.current=()=>{};};
 },[id,style,thumbnail,visible,pageVisible,width,safeSize]);
 return <span ref={root} data-material-id={id} data-material-state={thumbnail?"poster":status} style={{position:"relative",display:"inline-block",width:safeSize,maxWidth:"100%",aspectRatio:"1",verticalAlign:"middle"}}>
  {(thumbnail||status!=="ready"||!visible||!pageVisible)&&<picture><img key={`${id}-${style}`} src={`${assetBaseUrl}/${materialPosterPath(id,style)}`} alt={label} loading="lazy" width={safeSize} height={safeSize} onLoad={()=>setPosterError(false)} onError={()=>setPosterError(true)} style={{width:"100%",height:"100%",objectFit:"contain"}}/></picture>}
