...
export function ScienceControls({id,value,onChange,locale="fr"}:{id:ScienceId;value?:Partial<ScienceSettings>;onChange:(value:ScienceSettings)=>void;locale?:"fr"|"en"}){
 ...
 return <fieldset style={{border:0,padding:0,margin:"16px 0",minWidth:0,display:"grid",gap:14}}><legend style={{fontWeight:650,marginBottom:10}}>{s.title}</legend>{select(s.element,"element")}{id==="science-comparator"&&select(s.compare,"compareElement")}
