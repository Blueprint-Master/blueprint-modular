...
function frame(style:ScienceStyle){return style==="paper"?{bg:"#f1eee7",ink:"#14222b",muted:"#53646b",line:"#bec8c6"}:{bg:"#07111f",ink:"#f6fbff",muted:"#9bb0c2",line:"#294052"};}
...
function PeriodicTable({selected,locale,visualStyle,colorMode,layers,interactive,onSelect,uid}:{selected:ElementDatum;locale:"fr"|"en";visualStyle:ScienceStyle;colorMode:ScienceColorMode;layers:ScienceLayers;interactive:boolean;onSelect?:(symbol:string)=>void;uid:string}){
 ...
 return <svg role={interactive?"group":"img"} aria-label={locale==="fr"?"Tableau périodique interactif":"Interactive periodic table"} viewBox="0 0 760 470" style={{display:"block",width:"100%",height:"100%"}}>
  <defs><filter id={glow}><feGaussianBlur stdDeviation="4" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs><rect width="760" height="470" rx="28" fill={c.bg}/>
...
function Atom({element,locale,visualStyle,colorMode,layers,running,speed,uid}:{element:ElementDatum;locale:"fr"|"en";visualStyle:ScienceStyle;colorMode:ScienceColorMode;layers:ScienceLayers;running:boolean;speed:number;uid:string}){
 const c=frame(visualStyle),accent=tone(element,colorMode),dots=Math.min(28,Math.max(5,Math.ceil(Math.sqrt(element.atomicNumber)*2.5))),field=uid+"-field",lobe=uid+"-lobe",blur=uid+"-blur",soft=uid+"-soft",play=running?"running":"paused",duration=12/Math.max(.1,speed);
 const densityLobes=[
  {cx:174,cy:207,rx:62,ry:27,rotation:-26,delay:0},
  {cx:338,cy:296,rx:66,ry:29,rotation:-25,delay:-4},
  {cx:306,cy:168,rx:58,ry:25,rotation:61,delay:-8},
  {cx:213,cy:337,rx:61,ry:26,rotation:61,delay:-2},
 ];
 return <svg role="img" aria-label={(locale==="fr"?"Analyse atomique ":"Atomic analysis ")+element.name[locale]} viewBox="0 0 520 520" style={{display:"block",width:"100%",height:"100%"}}>
  <defs><radialGradient id={field}><stop stopColor={accent} stopOpacity=".55"/><stop offset=".45" stopColor={accent} stopOpacity=".14"/><stop offset="1" stopColor={accent} stopOpacity="0"/></radialGradient><radialGradient id={lobe}><stop stopColor="#fff" stopOpacity=".8"/><stop offset=".22" stopColor={accent} stopOpacity=".62"/><stop offset="1" stopColor={accent} stopOpacity="0"/></radialGradient><filter id={blur}><feGaussianBlur stdDeviation="10"/></filter><filter id={soft}><feGaussianBlur stdDeviation="2"/></filter></defs><rect width="520" height="520" rx="32" fill={c.bg}/>
  {layers.guides&&<g fill="none" stroke={c.line} strokeWidth="1">{Array.from({length:element.period},(_,i)=><ellipse key={i} cx="260" cy="252" rx={64+i*24} ry={28+i*12} transform={"rotate("+(i%2?58:-28)+" 260 252)"} strokeDasharray="4 7" opacity={.3+i*.07}/>)}</g>}
  {layers.structure&&<g><ellipse cx="260" cy="252" rx="185" ry="78" transform="rotate(-28 260 252)" fill={"url(#"+field+")"} filter={"url(#"+blur+")"}/><ellipse cx="260" cy="252" rx="170" ry="63" transform="rotate(61 260 252)" fill={"url(#"+field+")"} filter={"url(#"+blur+")"} opacity=".72"/><g data-science-density-lobes="4">{densityLobes.map((density,index)=><ellipse key={index} cx={density.cx} cy={density.cy} rx={density.rx} ry={density.ry} transform={"rotate("+density.rotation+" "+density.cx+" "+density.cy+")"} fill={"url(#"+lobe+")"} filter={"url(#"+soft+")"} style={{transformBox:"fill-box",transformOrigin:"center",animation:"science-density-bloom "+duration+"s cubic-bezier(.45,0,.25,1) infinite",animationDelay:(density.delay/Math.max(.1,speed))+"s",animationPlayState:play}}/>)}</g></g>}
  {layers.classification&&<g opacity=".9"><ellipse cx="260" cy="252" rx="172" ry="66" transform="rotate(-28 260 252)" fill="none" stroke={accent} strokeWidth="1.5" strokeDasharray="3 10"/><ellipse cx="260" cy="252" rx="152" ry="54" transform="rotate(61 260 252)" fill="none" stroke={accent} strokeOpacity=".7"/></g>}
  {layers.structure&&<g>{Array.from({length:dots},(_,i)=>{const a=i*2.399,r=7+Math.sqrt(i)*6.3;return <circle key={i} cx={260+Math.cos(a)*r} cy={252+Math.sin(a)*r} r={i%3===0?5.2:4.2} fill={i%2?accent:"#eef6ff"} fillOpacity={.72+(i%4)*.06} filter={"url(#"+soft+")"}/>;})}<circle cx="260" cy="252" r="34" fill="none" stroke={accent} strokeWidth="1.5"/></g>}
  {layers.identity&&<g><text x="36" y="54" fill={c.muted} fontSize="10" letterSpacing="2">{locale==="fr"?"MODÈLE DE DENSITÉ ÉLECTRONIQUE":"ELECTRON-DENSITY MODEL"}</text><text x="36" y="102" fill={c.ink} fontSize="40" fontWeight="700">{element.symbol}</text><text x="102" y="82" fill={c.ink} fontSize="16" fontWeight="600">{element.name[locale]}</text><text x="102" y="104" fill={c.muted} fontSize="11">Z = {element.atomicNumber}</text></g>}
  {layers.analysis&&<g transform="translate(36 424)"><rect width="448" height="62" rx="14" fill={accent} fillOpacity={visualStyle==="paper"?.09:.12} stroke={c.line}/><text x="18" y="24" fill={c.muted} fontSize="10">{locale==="fr"?"NOYAU":"NUCLEUS"}</text><text x="18" y="45" fill={c.ink} fontSize="13">{element.atomicNumber} p⁺</text><text x="132" y="24" fill={c.muted} fontSize="10">{locale==="fr"?"ÉLECTRONS (NEUTRE)":"ELECTRONS (NEUTRAL)"}</text><text x="132" y="45" fill={c.ink} fontSize="13">{element.atomicNumber} e⁻</text><text x="330" y="24" fill={c.muted} fontSize="10">{locale==="fr"?"BLOC":"BLOCK"}</text><text x="330" y="45" fill={accent} fontSize="13">{element.block}</text></g>}
  <style>{"@keyframes science-density-bloom{0%,100%{opacity:.08;transform:scale(.45)}18%{opacity:.35;transform:scale(.72)}42%{opacity:.92;transform:scale(1.08)}64%{opacity:.48;transform:scale(.9)}82%{opacity:.12;transform:scale(.58)}}@media(prefers-reduced-motion:reduce){svg *{animation:none!important}}"}</style>
 </svg>;
}
...
function Comparator({left,right,locale,visualStyle,colorMode,layers}:{left:ElementDatum;right:ElementDatum;locale:"fr"|"en";visualStyle:ScienceStyle;colorMode:ScienceColorMode;layers:ScienceLayers}){
 ...
 return <svg role="img" aria-label={left.name[locale]+" / "+right.name[locale]} viewBox="0 0 520 520" style={{display:"block",width:"100%",height:"100%"}}><rect width="520" height="520" rx="32" fill={c.bg}/>{layers.structure&&<path d="M260 52V468" stroke={c.line}/>}
