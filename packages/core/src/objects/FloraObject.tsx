...
import {FLORA_ASSET_PATH,floraBudget,floraPosterPath,type FloraId,type FloraStyle} from "./flora";
export interface FloraObjectProps{id:FloraId;label:string;size?:number;style?:FloraStyle;playing?:boolean;speed?:number;thumbnail?:boolean;assetBaseUrl?:string;}
/** Living botanical study. A fixed lazy SSR poster becomes one bounded canvas only in view. */
export function FloraObject({id,label,size=360,style="photorealistic",playing=true,speed=1,thumbnail=false,assetBaseUrl=FLORA_ASSET_PATH}:FloraObjectProps){
 ...
 useEffect(()=>{if(thumbnail||!visible||!pageVisible)return;const target=canvas.current;if(!target)return;let disposed=false;const media=window.matchMedia("(prefers-reduced-motion: reduce)");
  const constrained=window.matchMedia("(pointer: coarse)").matches||window.innerWidth<600||(navigator as Navigator&{connection?:{saveData?:boolean}}).connection?.saveData===true,budget=floraBudget(Math.min(width,safeSize),window.devicePixelRatio||1,constrained);
  ...
  void import("./flora-renderer").then(({drawFlora})=>{if(disposed)return;target.width=target.height=budget.size;const ctx=target.getContext("2d");if(!ctx)throw Error("Canvas unavailable");
   draw=()=>drawFlora(ctx,id,style,elapsed.current,budget.size);refresh.current=update;media.addEventListener("change",update);setStatus("ready");update();
 ...
 return <span ref={root} data-flora-id={id} data-flora-state={thumbnail?"poster":status} style={{position:"relative",display:"inline-block",width:safeSize,maxWidth:"100%",aspectRatio:"1",verticalAlign:"middle"}}>
  {(thumbnail||status!=="ready"||!visible||!pageVisible)&&<picture><img key={`${id}-${style}`} src={`${assetBaseUrl}/${floraPosterPath(id,style)}`} alt={label} loading="lazy" width={safeSize} height={safeSize} onLoad={()=>setPosterError(false)} onError={()=>setPosterError(true)} style={{width:"100%",height:"100%",objectFit:"contain"}}/></picture>}
