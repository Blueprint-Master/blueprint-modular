...
export async function GET(){return NextResponse.json({schemaVersion:1,objects:DISCOVERABLE_OBJECTS.map(o=>({...o,version:o.family==="space"?UNIVERSE_VERSION:o.version,
  previewPath:isWaterId(o.id)?`${WATER_ASSET_PATH}/${waterPosterPath(o.id,"photorealistic")}`:isFormId(o.id)?`${FORMS_ASSET_PATH}/${formPosterPath(o.id,"photorealistic")}`:isWeatherId(o.id)?`${WEATHER_ASSET_PATH}/${weatherPosterPath(o.id,"photorealistic")}`:o.family==="space"?`${UNIVERSE_ASSET_PATH}/${planetPosterPath(o.id as PlanetId,"photorealistic")}`:`/objects/vector-v1/${o.id}.png`,
  variants:o.family==="space"||o.family==="weather"||o.family==="forms"||o.family==="water"?["photorealistic","illustration"]:["vector"],animated:o.family==="space"||o.family==="weather"||o.family==="forms"||o.family==="water",
