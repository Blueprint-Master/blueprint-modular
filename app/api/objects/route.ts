...
import {isMaterialId,MATERIAL_ASSET_PATH,materialPosterPath} from "../../../packages/core/src/objects/materials";
import {isScienceId,SCIENCE_ASSET_PATH,sciencePosterPath} from "../../../packages/core/src/objects/science";
...
export async function GET(){return NextResponse.json({schemaVersion:1,objects:DISCOVERABLE_OBJECTS.map(o=>({...o,version:o.family==="space"?UNIVERSE_VERSION:o.version,
  previewPath:isScienceId(o.id)?`${SCIENCE_ASSET_PATH}/${sciencePosterPath(o.id,"midnight")}`:isMaterialId(o.id)?`${MATERIAL_ASSET_PATH}/${materialPosterPath(o.id,"photorealistic")}`:isFloraId(o.id)?`${FLORA_ASSET_PATH}/${floraPosterPath(o.id,"photorealistic")}`:isWaterId(o.id)?`${WATER_ASSET_PATH}/${waterPosterPath(o.id,"photorealistic")}`:isFormId(o.id)?`${FORMS_ASSET_PATH}/${formPosterPath(o.id,"photorealistic")}`:isWeatherId(o.id)?`${WEATHER_ASSET_PATH}/${weatherPosterPath(o.id,"photorealistic")}`:o.family==="space"?`${UNIVERSE_ASSET_PATH}/${planetPosterPath(o.id as PlanetId,"photorealistic")}`:`/objects/vector-v1/${o.id}.png`,
  variants:o.family==="science"?["midnight","paper","transparent"]:o.family==="space"||o.family==="weather"||o.family==="forms"||o.family==="water"||o.family==="flora"||o.family==="materials"?["photorealistic","illustration"]:["vector"],animated:o.family==="space"||o.family==="weather"||o.family==="forms"||o.family==="water"||o.family==="flora"||o.family==="materials"||o.family==="science",
  ...(o.family==="science"?{assetBasePath:SCIENCE_ASSET_PATH}:{}),
  ...(o.family==="materials"?{assetBasePath:MATERIAL_ASSET_PATH}:{}),
