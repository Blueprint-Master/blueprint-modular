...
const PREVIEW_BY_SLUG: Record<string, React.ReactNode> = {
  ...
  highlightbox: <PreviewBox><HighlightBox value={1} label="RTB" title="Exemple" momentDescription="Aujourd'hui" rtbPoints={["Cible"]} targetPoints="100%" /></PreviewBox>,
  altairchart: <PreviewBox><span className="text-sm" style={{ color: "var(--bpm-text-secondary)" }}>bpm.altairchart</span></PreviewBox>,
  plotlychart: <PreviewBox><span className="text-sm" style={{ color: "var(--bpm-text-secondary)" }}>bpm.plotlychart</span></PreviewBox>,
  pdfviewer: <PreviewBox><span className="text-sm" style={{ color: "var(--bpm-text-secondary)" }}>bpm.pdfviewer</span></PreviewBox>,
  filterpanel: <PreviewBox><FilterPanel filters={[{ key: "s", label: "Statut", type: "select", options: [{ value: "a", label: "A" }] }]} values={{}} onChange={() => {}} onReset={() => {}} /></PreviewBox>,
  confirmmodal: <PreviewBox><span className="text-sm" style={{ color: "var(--bpm-text-secondary)" }}>bpm.confirmModal</span></PreviewBox>,
  toast: <Toast id={1} message="Notification" type="info" title={null} pageName={null} pageIcon={null} onClose={() => {}} />,
  pagelayout: <PreviewBox><span className="text-sm" style={{ color: "var(--bpm-text-secondary)" }}>bpm.pageLayout</span></PreviewBox>,
  scrollcontainer: <ScrollContainer maxHeight={80}><div style={{ padding: 8 }}><Text>Contenu scrollable</Text></div></ScrollContainer>,
  labelvalue: <LabelValue label="Nom" value="Valeur" />,
  spinnerdot: <SpinnerDot />,
  titlebpm: <Title level={2}>Titre</Title>,
  markdown: <Markdown text="**Gras** et *italique*" />,
  codeeditor: <PreviewBox><CodeEditor value="// code" onChange={() => {}} height={80} /></PreviewBox>,
  ...
  promptinput: <PreviewBox><PromptInput value="" onChange={() => {}} onSubmit={() => {}} placeholder="Prompt..." /></PreviewBox>,
  streamingtext: <StreamingText content="Texte en flux" isStreaming={false} />,
  diffviewer: <PreviewBox><DiffViewer original="a" modified="b" mode="unified" /></PreviewBox>,
  modelselector: <PreviewBox><ModelSelector models={[{ id: "m1", label: "Modèle 1", provider: "Provider" }]} selected="m1" onChange={() => {}} /></PreviewBox>,
};
