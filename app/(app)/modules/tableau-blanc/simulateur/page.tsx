                      ...
                      {isEditing ? (
                        <>
                          <textarea
                            ...
                            className="flex-1 min-h-[60px] w-full rounded border px-2 py-1.5 text-sm resize-y"
                            style={{ borderColor: "var(--bpm-border)", background: "var(--bpm-bg-primary)", color: "var(--bpm-text-primary)" }}
                            autoFocus
                          />
                        ...
                        <>
                          ...
                          {/* Actions au survol */}
                          <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex flex-wrap gap-1">
                            <button type="button" onClick={() => handleEdit(p.id, p.content)} className="p-1 rounded hover:bg-black/10" title="Modifier" aria-label="Modifier">✏️</button>
                            <button type="button" onClick={() => handleDelete(p.id)} className="p-1 rounded hover:bg-black/10" title="Supprimer" aria-label="Supprimer">🗑️</button>
                            {COLUMNS.filter((c) => c.id !== p.column).map((c) => (
                              <button key={c.id} type="button" onClick={() => handleMove(p.id, c.id)} className="p-1 rounded hover:bg-black/10 text-xs" title={`Déplacer vers ${c.label}`}>
                                → {c.label}
                              </button>
                            ))}
                          </div>
