...
export default function WikiHistoryPage() {
  ...
  const handleRestore = async () => {
    ...
    try {
      ...
      else setError(t.history.errRestoreVersion);
    } catch {
      setError(t.history.errRestore);
    } finally {
      setRestoring(false);
  ...
  if (loading) return <p style={{ color: "var(--bpm-text-secondary)" }}>{t.common.loadingEllipsis}</p>;
  if (error) {
    return (
      <Panel variant="error" title={t.common.error}>
        {error}
        <Link href={`/modules/wiki/${slug}`} className="block mt-2 underline" style={{ color: "var(--bpm-accent-cyan)" }}>{t.common.backToArticlePlain}</Link>
      </Panel>
    );
  }
    ...
    <div className="doc-page">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        ...
        <Link href={`/modules/wiki/${slug}`}>
          <Button variant="outline" size="small">{t.common.backToArticle}</Button>
        </Link>
      </div>
            ...
            {revisions.map((r) => (
              <li key={r.id} className="text-sm flex flex-wrap items-center gap-2 py-2 border-b border-[var(--bpm-border)] last:border-b-0">
                <span className="font-medium" style={{ color: "var(--bpm-text-primary)" }}>
                  {new Date(r.createdAt).toLocaleString(t.common.locale)}
                </span>
                ...
                <div className="flex gap-1 mt-1 flex-shrink-0">
                  ...
                  >
                    {compareId === r.id ? t.history.deselect : t.history.compare}
                  </button>
                </div>
              </li>
            ))}
        ...
        >
          <Card variant="outlined" title={t.history.restoreDialogTitle} className="max-w-md w-full">
            ...
            <label className="block mb-4">
              <span className="block text-sm mb-1" style={{ color: "var(--bpm-text-secondary)" }}>{t.history.restoreNoteLabel}</span>
              <input
                type="text"
                value={restoreNote}
                onChange={(e) => setRestoreNote(e.target.value)}
                placeholder={t.history.restoreNotePlaceholder}
      ...
      <nav className="doc-pagination mt-8">
        <Link href={`/modules/wiki/${slug}`} style={{ color: "var(--bpm-accent-cyan)" }}>{t.common.backToArticle}</Link>
        <Link href={`/modules/wiki/${slug}/edit`} style={{ color: "var(--bpm-accent-cyan)" }}>{t.common.edit}</Link>
      </nav>
    </div>
  );
}
