    ...
    return (
      <div className="doc-page">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
          ...
          <Button size="small" variant="primary" onClick={() => setFormOpen(true)}>+ Nouvel événement</Button>
        </div>

        <div className="flex flex-wrap items-center gap-2 mb-4">
          <span className="text-xs font-medium" style={{ color: "var(--bpm-text-secondary)" }}>Filtrer :</span>
          <Chip label="Tous" variant={filterCouleur === null ? "primary" : "default"} onClick={() => setFilterCouleur(null)} />
        ...
        <div className="grid grid-cols-7 gap-px rounded-lg overflow-hidden calendar-month-grid" style={{ background: "var(--bpm-border)" }}>
          ...
          {Array.from({ length: totalCells }, (_, i) => {
            ...
            const dayEvs = dateStr ? filteredEvents.filter((e) => e.date === dateStr) : [];
            ...
            return (
              ...
              >
                {isCurrentMonth ? (
                  <button
                    type="button"
                    className="calendar-month-day-num text-xs font-medium text-left w-full px-1 py-0.5 rounded hover:bg-[var(--bpm-bg-secondary)]"
                    style={{ color: isToday ? "var(--bpm-accent)" : undefined, fontWeight: isToday ? 600 : undefined }}
                    onClick={() => { setFocusDate(new Date(year, month, dayNum)); setSelectedEvent(dayEvs[0] ?? null); }}
                  >
                    {dayNum}
                  </button>
                ) : (
                  <span className="calendar-month-day-num text-xs font-medium">{""}</span>
                )}
                <div className="flex-1 space-y-0.5 overflow-auto flex flex-wrap gap-0.5 content-start p-1">
                  {dayEvs.slice(0, 5).map((e, j) => (
                    <span key={j} className="block w-2 h-2 rounded-full shrink-0" style={{ background: e.couleur ?? "var(--bpm-accent-cyan)" }} title={e.titre} />
                  ))}
                  {dayEvs.length > 5 && <span className="text-xs" style={{ color: "var(--bpm-text-secondary)" }}>+{dayEvs.length - 5}</span>}
                </div>
              </div>
            );
          })}
        ...
        <p className="mt-4 text-sm" style={{ color: "var(--bpm-text-secondary)" }}>
          Cliquez sur un jour pour afficher les événements. Sur mobile, utilisez les vues Jour / Semaine / Mois.
        </p>
